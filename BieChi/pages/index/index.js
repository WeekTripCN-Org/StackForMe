...
Page({
  ...
  getBanners() {
    this.banner.queryAsync({is_show: !0}).then(data => {
      console.log(data)
      if(data.meta.code == 0) {
  ...
  getList() {
    ...
    this.goods.queryAsync(params).then(data => {
      console.log(data)
      if(data.meta.code == 0) {
        data.data.items.forEah(n => n.thumb_url = App.renderImage(n.images[0] && n.images[0].path))
